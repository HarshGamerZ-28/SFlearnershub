...
import {
  Search, Menu, X, ChevronDown, Moon, Sun, LogIn
} from "lucide-react";
...
export default function Navbar() {
  ...
  return (
    ...
    >
      ...
      <div
        className={`lg:hidden fixed inset-0 top-14 sm:top-16 z-40 bg-white/98 dark:bg-dark-900/98 backdrop-blur-xl transition-all duration-300 ease-out ${
          open ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"
        }`}
      >
        <div className="h-full overflow-y-auto px-4 py-4 pb-8">
          <nav className="space-y-1">
            <Link href="/" onClick={() => setOpen(false)} className="flex items-center px-3 py-3 rounded-xl text-base font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-white/5 transition-all">
              Home
            </Link>

            <div className="pt-3 pb-1 px-3 text-xs font-bold text-slate-400 uppercase tracking-wider">
              Blog Categories
            </div>
            <div className="grid grid-cols-1 gap-0.5">
              {BLOG_CATEGORIES.map((item) => (
                <Link
                  ...
                  className="flex items-center px-3 py-2.5 rounded-xl text-sm text-slate-600 dark:text-slate-400 hover:text-brand-600 dark:hover:text-brand-400 hover:bg-slate-100 dark:hover:bg-white/5 transition-all"
                >
                  {item.name}
                </Link>
              ))}
            ...
            {[
              ...
              <Link
                ...
                href={link.href}
                onClick={() => setOpen(false)}
                className="flex items-center px-3 py-3 rounded-xl text-base font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-white/5 transition-all"
              >
                {link.label}
              </Link>
            ))}
            ...
            <Link
              href="/auth/login"
              onClick={() => setOpen(false)}
              className="flex items-center gap-2 px-3 py-3 mt-2 rounded-xl text-base font-medium text-brand-600 dark:text-brand-400 border border-brand-500/30 hover:bg-brand-500/5 transition-all"
            >
              <LogIn size={18} />
              Sign In
            </Link>
